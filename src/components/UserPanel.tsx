...
const WeightTooltip: React.FC<TooltipProps> = ({ active, payload, goalWeight }) => {
  if (!active || !payload?.length) return null;
  const d = payload[0].payload;
  return (
    <div style={tooltipBox}>
      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#aeaeb2", margin: "0 0 4px" }}>{d.fullDate}</p>
      <p style={{ fontSize: 15, fontWeight: 700, color: "#10b981", margin: 0 }}>{d.weight} kg</p>
      {goalWeight != null && (
        <p style={{ fontSize: 12, color: "#c2410c", margin: "4px 0 0", fontWeight: 500 }}>Goal: {goalWeight} kg</p>
      )}
    </div>
  );
};

const BMITooltip: React.FC<TooltipProps> = ({ active, payload }) => {
  if (!active || !payload?.length) return null;
  const d = payload[0].payload;
  return (
    <div style={tooltipBox}>
      <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#aeaeb2", margin: "0 0 4px" }}>{d.fullDate}</p>
      <p style={{ fontSize: 15, fontWeight: 700, color: "#14b8a6", margin: 0 }}>BMI {d.bmi.toFixed(1)}</p>
    </div>
  );
};
...
function bmiCategoryBadge(cat?: string) {
...
/** Client-side trend insight — no API call */
function buildInsight(
  ...
  const days  = Math.max(1, Math.round((chartData[chartData.length - 1].timestamp - chartData[0].timestamp) / 86_400_000));
...
export function UserPanel() {
  ...
  const weightProgress  = progressRange === 0
    ? 0 : Math.max(0, Math.min(100, (currentProgress / progressRange) * 100));


  const daysSinceLastEntry = useMemo(() => {
    if (!allChartData.length) return null;
    return Math.floor((Date.now() - allChartData[allChartData.length - 1].timestamp) / 86_400_000);
  }, [allChartData]);
  ...
  const handleBodySubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isNaN(rawW) || rawW <= 0 || !savedHeight) { setIsSubmitting(false); return; }
    ...
    const entry = {
      value: Math.round(bmi * 10) / 10, category,
      height: savedHeight, weight: Number(wKg.toFixed(1)),
      unit: savedHeightUnit, date: new Date().toISOString(),
    };

    const res = await fetch("/api/user/metrics", {
      method: "POST", headers: { "Content-Type": "application/json" }, credentials: "include",
      body: JSON.stringify({ latestBMI: entry, height: savedHeight, weight: entry.weight, unit: savedHeightUnit, bmiHistoryEntry: entry }),
    });
  ...
  const handleGoalSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (isNaN(gw) || gw <= 0) { setIsSubmitting(false); return; }
  ...
  const SidebarContent = () => (
    <>
      <div className={s.sidebarHead}>
        ...
        <div className={s.sidebarUser}>
          ...
          <div className={s.userMeta}>
            ...
            <div className={s.userRole}>Health Member</div>
          </div>
        </div>
      </div>
      ...
      <nav className={s.sidebarNav}>
        ...
        <button className={`${s.navBtn} ${s.navBtnActive}`} onClick={() => setIsMobileSidebarOpen(false)}>
          <Activity size={15} />Body Stats
        </button>

        <div className={s.navSection} style={{ marginTop: "0.5rem" }}>Quick Actions</div>
      </nav>

      <div className={s.quickActions}>
        <button className={s.quickActionBtn} onClick={() => { setIsMobileSidebarOpen(false); window.location.hash = "#ai-advisor"; }}>
          <Brain size={14} style={{ color: "#8b5cf6" }} />AI Advisor
        </button>
      </div>
  ...
  const RangeFilter = ({ value, onChange }: { value: RangeKey; onChange: (r: RangeKey) => void }) => (
  ...
  const ChartHeader = ({
    icon, title, sub, range, onRange, action, onAction,
  }: {
    ...
    <div className={s.chartTopRow}>
      <div>
        <div className={s.sectionTitle}>{icon}{title}</div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "0.625rem", flexWrap: "wrap", justifyContent: "flex-end" }}>
        {action && onAction && (
          <button className={s.sectionAction} onClick={onAction}>{action}</button>
        )}
  ...
  /* ── Dot timeline for sparse data (< 4 points) ─────────────────────────── */
  const DotTimeline = ({ data }: { data: ChartPoint[] }) => (
    <div className={s.dotTimeline}>
      {[...data].reverse().map((pt, i) => {
        const isLatest = i === 0;
        return (
          <div key={pt.timestamp} className={s.dotTimelineItem}>
            <div className={`${s.dotTimelineDot} ${isLatest ? s.dotTimelineDotLatest : ""}`} />
            <div className={s.dotTimelineContent}>
              <div className={s.dotTimelineDate}>{pt.fullDate}</div>
              <div className={s.dotTimelineVals}>
                <div>
                  <div className={s.dotTimelineValNum}>{pt.weight} kg</div>
                  <div className={s.dotTimelineValLabel}>Weight</div>
                </div>
                <div>
                  <div className={s.dotTimelineValNum}>{pt.bmi.toFixed(1)}</div>
                  <div className={s.dotTimelineValLabel}>BMI</div>
                </div>
                {goalWeight && (
                  <div>
                    <div className={s.dotTimelineValNum} style={{ color: "#c2410c" }}>
                      {Math.abs(pt.weight - goalWeight).toFixed(1)} kg
                    </div>
                    <div className={s.dotTimelineValLabel}>From goal</div>
                  </div>
                )}
              </div>
            </div>
      ...
      <div className={s.main}>
        ...
        {!isDesktop && (
          <div className={s.topbar}>
            ...
            <button className={s.topbarMenuBtn} onClick={() => setIsMobileSidebarOpen(true)} aria-label="Open menu">
              <Menu size={17} />
            </button>
          </div>
        )}
        ...
        <div className={s.content}>
          ...
          <div className={s.statsRow}>
            ...
            {/* BMI card with scale bar */}
            <div className={s.statCard}>
              <div className={s.statCardTop}>
                <span className={s.statLabel}>Current BMI</span>
                <div className={s.statIcon}><Scale size={13} /></div>
              </div>
              {latestBMI ? (
                <>
                  <div className={s.statValue}>{latestBMI.value.toFixed(1)}</div>
                  <span className={`${s.badge} ${bmiCategoryBadge(latestBMI.category)}`}>{latestBMI.category}</span>
                  {/* Inline BMI scale bar */}
                  ...
                  <div className={s.bmiScaleLabel}>Under · Normal · Over · Obese</div>
                </>
              ) : (
                <div className={s.statSub}>No data yet</div>
              )}
            </div>
            ...
            <div className={s.statCard}>
              <div className={s.statCardTop}>
                ...
                <div className={s.statIcon}><Weight size={13} /></div>
              </div>
              {latestBMI ? (
                <>
                  <div className={s.statValue}>
                    {latestBMI.weight}<span className={s.statUnit}>kg</span>
                  </div>
                  <div className={s.statSub}>{latestBMI.height}{latestBMI.unit === "metric" ? " cm" : " in"} tall</div>
                </>
              ) : <div className={s.statSub}>No data yet</div>}
            </div>
            ...
            <div className={s.statCard}>
              <div className={s.statCardTop}>
                ...
                <div className={s.statIcon}><Target size={13} /></div>
              </div>
              {goalWeight ? (
                <>
                  <div className={s.statValue}>
                    {goalWeight}<span className={s.statUnit}>kg</span>
                  </div>
                  <div className={s.statSub}>
                    {currentWeight > 0 ? `${Math.abs(currentWeight - goalWeight).toFixed(1)} kg remaining` : "Set"}
                  </div>
                </>
              ) : <div className={s.statSub}>Not set yet</div>}
            </div>
            ...
            <div className={s.statCard}>
              <div className={s.statCardTop}>
                ...
                <div className={s.statIcon}><TrendingUp size={13} /></div>
              </div>
              <div className={s.statValue}>{bmiHistory.length}</div>
              <div className={s.statSub}>BMI recordings</div>
              {daysSinceLastEntry !== null && (
                <div className={s.statSub} style={{ marginTop: 2 }}>
          ...
          {/* ── Insight banner (computed client-side) ── */}
          {allChartData.length > 0 && (
            <div className={s.insightBanner}>
              <div className={s.insightIcon}><Sparkles size={17} /></div>
              <div>
          ...
          {!bmiHistory.length && !latestBMI && (
            <div style={{ background: "#fff", border: "1px solid #e8e8ed", borderRadius: "1.125rem", marginBottom: "1.25rem" }}>
              <div className={s.emptyState}>
                <div className={s.emptyIcon}><Activity size={22} /></div>
                <div className={s.emptyTitle}>Start tracking your health</div>
                <div className={s.emptyBody}>Calculate your BMI once to unlock trend graphs, goal tracking, and AI guidance.</div>
                <button className={s.emptyBtn} onClick={() => { window.location.hash = "#bmi"; }}>
          ...
          {/* ── Goal Progress — full width, big bar ── */}
          <div className={s.goalCard} style={{ marginBottom: "1.25rem" }}>
            <div className={s.sectionHead}>
              <div>
                <div className={s.sectionTitle}><Target size={16} style={{ color: "#10b981" }} />Goal Progress</div>
                <div className={s.sectionSub}>
                  {goalWeight ? `${startWeight} kg → ${goalWeight} kg` : "Set a target weight to track your progress"}
                </div>
              </div>
            ...
            {goalWeight && currentWeight > 0 ? (
              <>
                ...
                {/* Big progress bar */}
                <div className={s.bigProgressWrap}>
                  <div className={s.bigProgressLabels}>
                    <span className={s.bigProgressLeft}>{startWeight} kg</span>
                    <span className={s.bigProgressPct}>{Math.round(weightProgress)}% complete</span>
                    <span className={s.bigProgressRight}>{goalWeight} kg</span>
                  </div>
                  <div className={s.bigProgressTrack}>
                    <div className={s.bigProgressFill} style={{ width: `${weightProgress}%` }}>
                      {weightProgress >= 8 && (
                        <span className={s.bigProgressLabel}>{currentWeight} kg</span>
                      )}
                    </div>
                  </div>
                  <div className={s.bigProgressSubs}>
                ...
                {weightProgress >= 100 && (
                  <div style={{ display: "flex", alignItems: "center", gap: "0.375rem", marginTop: "1rem", fontSize: "0.8125rem", fontWeight: 600, color: "#15803d" }}>
                    <CheckCircle2 size={15} />Goal reached!
                  </div>
                )}
              </>
            ) : (
              <div className={s.setGoalCta}>
                <div className={s.setGoalCtaIcon}><Target size={22} /></div>
                <div className={s.setGoalCtaText}>Set a goal weight to start tracking your progress toward it.</div>
                <button className={s.sectionAction} style={{ marginTop: "0.25rem" }} onClick={openGoalForm}>Set goal →</button>
              </div>
            )}
          </div>
          ...
          <div className={s.chartCard}>
            <ChartHeader
              icon={<Weight size={16} style={{ color: "#10b981" }} />}
              ...
              <div className={s.chartMeta}>
                ...
                {goalWeight && (
                  <div>
                    ...
                    <div className={s.chartMetaValue} style={{ color: "#c2410c" }}>{goalWeight} kg</div>
                  </div>
                )}
                <div>
                  <div className={s.chartMetaLabel}>Change</div>
                  <div className={s.chartMetaValue} style={{ color: currentWeight <= startWeight ? "#15803d" : "#c2410c" }}>
            ...
            {!allChartData.length ? (
              <div className={s.emptyState}>
                <div className={s.emptyIcon}><Weight size={20} /></div>
              ...
              /* Dot timeline for sparse data */
              <DotTimeline data={weightChartData} />
            ) : (
              <ResponsiveContainer width="100%" height={280}>
                <AreaChart data={weightChartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  <XAxis dataKey="date" tick={{ fill: "#aeaeb2", fontSize: 11, fontFamily: "-apple-system" }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                  <YAxis tick={{ fill: "#aeaeb2", fontSize: 11, fontFamily: "-apple-system" }} tickLine={false} axisLine={false} domain={["auto", "auto"]} width={36} />
                  <Tooltip content={<WeightTooltip goalWeight={goalWeight} />} cursor={{ stroke: "#e8e8ed", strokeWidth: 1 }} />
                  {goalWeight && (
                    <ReferenceLine y={goalWeight} stroke="#c2410c" strokeDasharray="5 4" strokeWidth={1.5}
                      label={{ value: `Goal ${goalWeight}kg`, position: "insideTopRight", fill: "#c2410c", fontSize: 11, fontWeight: 600 }} />
                  )}
                  <Area type="monotone" dataKey="weight" stroke="#10b981" strokeWidth={2.5} fill="url(#wGrad)"
                    dot={<CustomDot />} activeDot={{ r: 6, fill: "#10b981", stroke: "#fff", strokeWidth: 2 }} />
                </AreaChart>
              </ResponsiveContainer>
            )}
          </div>
          ...
          <div className={s.chartCard}>
            <ChartHeader
              icon={<TrendingUp size={16} style={{ color: "#14b8a6" }} />}
              title="BMI History"
            ...
            {!allChartData.length ? (
              <div className={s.emptyState}>
                <div className={s.emptyIcon}><TrendingUp size={20} /></div>
              ...
              <DotTimeline data={bmiChartData} />
            ) : (
              <ResponsiveContainer width="100%" height={240}>
                <AreaChart data={bmiChartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
                  <defs>
                  ...
                  <XAxis dataKey="date" tick={{ fill: "#aeaeb2", fontSize: 11, fontFamily: "-apple-system" }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                  <YAxis tick={{ fill: "#aeaeb2", fontSize: 11, fontFamily: "-apple-system" }} tickLine={false} axisLine={false} domain={["auto", "auto"]} width={36} />
                  <Tooltip content={<BMITooltip />} cursor={{ stroke: "#e8e8ed", strokeWidth: 1 }} />
                  <Area type="monotone" dataKey="bmi" stroke="#14b8a6" strokeWidth={2.5} fill="url(#bGrad)"
                    dot={{ fill: "#14b8a6", r: 4, stroke: "#fff", strokeWidth: 2 }}
              ...
              <div className={s.historyHead}>
                <div>
                  <div className={s.sectionTitle} style={{ marginBottom: 0 }}>
                    <Scale size={16} style={{ color: "#10b981" }} />Entry History
                  </div>
                  <div className={s.sectionSub}>{bmiHistory.length} total recording{bmiHistory.length !== 1 ? "s" : ""}</div>
                </div>
                <button className={s.tableCollapseBtn} onClick={() => setHistoryOpen(h => !h)}>
                  {historyOpen ? <><ChevronUp size={15} />Hide</> : <><ChevronDown size={15} />View all</>}
                </button>
              </div>
                    ...
                    <tbody>
                      {[...allChartData].reverse().map((pt) => (
                        <tr key={pt.timestamp}>
                          ...
                          <td>
                            <span className={`${s.badge} ${bmiCategoryBadge(
                              pt.bmi < 18.5 ? "Underweight" :
                              pt.bmi < 25   ? "Normal Weight" :
                              pt.bmi < 30   ? "Overweight" : "Obese"
                            )}`}>
                              {pt.bmi < 18.5 ? "Underweight" :
                               pt.bmi < 25   ? "Normal Weight" :
                               pt.bmi < 30   ? "Overweight" : "Obese"}
                          ...
                          <td style={{ color: "#636366" }}>
                            {latestBMI?.height}{latestBMI?.unit === "metric" ? " cm" : " in"}
                          </td>
                        </tr>
                      ))}
                    </tbody>
        ...
        <>
          ...
          <aside className={s.drawerSidebar}>
            <div className={s.drawerHead}>
              ...
              <button className={s.drawerClose} onClick={() => setIsMobileSidebarOpen(false)} aria-label="Close">
                <X size={13} />
              </button>
            </div>
            <SidebarContent />
      ...
      {isStatsFormOpen && (
        <>
          ...
          <aside className={s.formDrawer}>
            <div className={s.drawerHead}>
              <div>
                <div className={s.drawerTitle}>{activeForm === "body" ? "Log Weight Entry" : "Set Goal Weight"}</div>
                <div className={s.drawerSub}>{activeForm === "body" ? "Record your current weight." : "Set your target weight."}</div>
              </div>
              <button className={s.drawerClose} onClick={() => setIsStatsFormOpen(false)} aria-label="Close">
                <X size={13} />
              </button>
            </div>
              ...
              {activeForm === "body" ? (
                <form onSubmit={handleBodySubmit}>
                  {savedHeight > 0 && (
                    <div className={s.formHint}>
                      <Scale size={13} />
                      Using saved height: {savedHeight}{savedHeightUnit === "metric" ? " cm" : " ft"}
                    </div>
                  )}
                  <div className={s.formGroup}>
                    ...
                    <div className={s.formInputRow}>
                      <input
                        ...
                        className={s.formInput} placeholder="Enter weight"
                        value={bodyStatsForm.weight}
                        onChange={e => setBodyStatsForm(p => ({ ...p, weight: e.target.value }))}
                      />
                      <select
                        className={s.formSelect}
                        value={bodyStatsForm.weightUnit}
                        onChange={e => setBodyStatsForm(p => ({ ...p, weightUnit: e.target.value as BodyStatsForm["weightUnit"] }))}
                      >
                  ...
                  <button type="submit" className={s.submitBtn} disabled={isSubmitting}>
                    {isSubmitting ? <><div className={s.spinner} />Saving…</> : <><Save size={15} />Save Entry</>}
                  </button>
                </form>
              ) : (
                <form onSubmit={handleGoalSubmit}>
                  <div className={s.formGroup}>
                    ...
                    <input
                      type="number" min="0" step="0.1" required
                      className={s.formInput} placeholder="e.g. 72"
                      value={goalWeightInput}
                      onChange={e => setGoalWeightInput(e.target.value)}
                    />
                  </div>
                  <button type="submit" className={s.submitBtn} disabled={isSubmitting}>
                    {isSubmitting ? <><div className={s.spinner} />Saving…</> : <><Target size={15} />Set Goal</>}
                  </button>
                </form>
              )}
            </div>
